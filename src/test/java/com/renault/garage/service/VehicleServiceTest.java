...
class VehicleServiceTest {
    ...
    void shouldRejectWhenQuotaExceeded() {
        ...
        VehicleService service = new VehicleService(repo, kafka);

        Garage g = Garage.builder().id(1L).name("A").address("B").telephone("C").email("d@e.com").build();
        Vehicle v = Vehicle.builder().garage(g).brand("Renault").modele("Clio").anneeFabrication(2020).typeCarburant(FuelType.ESSENCE).build();
